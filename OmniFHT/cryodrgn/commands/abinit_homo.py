...
def save_checkpoint(
    ...
    model.eval()
    vol = model.eval_volume(lattice.coords, lattice.D, lattice.circle_mask_radius, lattice.extent, norm)
...
def pretrain(model, lattice, optim, batch, tilt=None):
    ...
    mask = lattice.get_circular_mask(lattice.circle_mask_radius)
    kz = (lattice.kz[0]).to(lattice.coords_odt.device)
    def gen_slice(R):
    ...
    if tilt is not None:
        ...
        # print(y.shape, kz[mask].unsqueeze(0).unsqueeze(0).shape)
        loss = F.mse_loss(gen_slice(rot), y*kz[mask].unsqueeze(0))
    loss.backward()
    optim.step()
    return loss.item()
...
def make_model(args, D: int):
    ...
    activation = {"relu": nn.ReLU, "leaky_relu": partial(nn.LeakyReLU, negative_slope=0.02)}[args.activation]
...
def main(args):
    ...
    model.eval()
    vol = model.eval_volume(lattice.coords, lattice.D, lattice.circle_mask_radius, lattice.extent, tuple(data.norm))
    MRCFile.write(out_mrc, vol)
